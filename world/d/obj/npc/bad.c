// bad.c -- fugitive NPC for the 逃犯 (fugitive bounty) quest. Ported from
// xyj2006 /d/obj/npc/bad.c. Spawned & scaled by /d/obj/bad.c. Scales to the
// assigning player, wanders within its spawn area, flees if unkilled ~480s.
// Deviations (Decision B + bug fixes): absent skill bainiao-jian->damo-jian,
// absent skill balei->xiaoyaoyou, fixed typos baieli->xiaoyaoyou, dagonforce->dragonforce.
  ...
  set("chat_msg_combat", ({
    ...
    switch(random(3)){    
	    ...
	    (: cast_spell, "juanbi" :),
           ...
           set_skill("sword",skillmax+random(skillmax/20));
           set_skill("damo-jian",skillmax+random(skillmax/20));
           set_skill("spells",skillmax+random(skillmax/20));
           set_skill("dodge",skillmax+random(skillmax/20));
